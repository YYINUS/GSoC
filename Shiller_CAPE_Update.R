...
path <- "sandbox/Shiller_CAPE.xls"

# Add mode = "wb". The downloaded file is corrupted for me without it --- YI
download.file(Shiller_CAPE_file, destfile = path, mode = "wb")
...
Shiller_CAPE <- xlsx::read.xlsx(path, sheetIndex = "Data", startRow = 8)

# Ensure all input are numeric
# Columns were initially formatted as character. Very hard to convert into numeric after the xts object is formed.
# Hence I convert them here --- YI
Shiller_CAPE <- sapply(Shiller_CAPE,as.numeric)

# Remove all empty columns
# There are empty columns between columns with values. Hence it is necessary to remove empty first --- YI
library(dplyr)
Shiller_CAPE <- as.data.frame(Shiller_CAPE)
